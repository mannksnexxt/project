...
function createBomb() {
    ...
    while (!bombCreated) { //пока бомбу не создали
        ...
        let bomb_cell = document.querySelector('.cell-' + bomb_y + '-' + bomb_x);
        let bomb_cell_classes = bomb_cell.classList;

        // проверка на змейку и на еду
        if (!bomb_cell_classes.contains('snake-unit') && !bomb_cell_classes.contains('food-unit')) {
            let classes = '';
...
function haveBomb(unit) {
    ...
    if (unit) {
        if (unit.classList.contains('bomb-unit')) check = true;
    }
